...
void Display2() {
    ...
    for (t = -pi + ratia; t < pi; t += ratia) {
        ...
        glVertex2f(x - ratia, y);
    }
    glEnd();
}


// TO DO : change the second part of the if
...
void Display3() {
    ...
    for (t = -pi / 2 + ratia; t < -pi / 2.4; t += ratia) {
...
void Display4() {
    double pi = 4 * atan(1.0);
    double ratia = 0.05;
    double a = 0.1, b = 0.2, x, y, t;


    glColor3f(1, 0.1, 0.1);
    glBegin(GL_LINE_STRIP);
    for (t = -4 * pi + ratia; t < 4 * pi; t += ratia) {
        x = a * t - b * sin(t);
        y = a - b * cos(t);
...
void Display7() {
    double pi = 4 * atan(1.0);
    double ratia = 0.001;
    double a = 0.4, r, t, x, y;


    glColor3f(1, 0.1, 0.1);
    glBegin(GL_LINE_STRIP);
    for (t = pi / 4 - ratia; t > -pi / 4; t -= ratia) {
        r = -a * sqrt(2 * (2 * (cos(t) * cos(t)) - 1));
...
void Display8() {
    double pi = 4 * atan(1.0);
    double ratia = 0.05;
    double e = 2.718, a = 0.02, r, t, x, y;


    glColor3f(1, 0.1, 0.1);
    glBegin(GL_LINE_STRIP);
    for (t = 0; t < 100 * ratia; t += ratia) {
        r = a * pow(e, 1 + t);
